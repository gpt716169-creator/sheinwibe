...
export default function ActiveOrders({ orders, onGoToOrders }) {
  ...
  const getSmartStatus = (order) => {
    ...
    if (diffMinutes >= 2 * 24 * 60) return { text: 'Ждем SHEIN', color: 'text-purple-400 bg-purple-400/10 border-purple-400/20' };
  ...
  return (
    <div className="mt-6 animate-fade-in">
        <div className="flex justify-between items-end mb-2 px-1">
            <h3 className="text-white font-bold text-sm uppercase tracking-wider">Активный заказ</h3>
            <button onClick={onGoToOrders} className="text-primary text-xs font-bold">Все</button>
        </div>

        <div 
            onClick={onGoToOrders}
            className="bg-gradient-to-br from-[#1c2636] to-[#151c28] border border-white/10 p-3 rounded-2xl shadow-xl cursor-pointer active:scale-[0.99] transition-transform relative overflow-hidden group"
        >
            {/* Декоративный фон */}
            ...
            <div className="flex justify-between items-center relative z-10">
                ...
                {/* СЛЕВА: Картинки товаров */}
                <div className="flex -space-x-3 pl-1">
                    {(latestOrder.order_items || []).slice(0, 3).map((item, idx) => (
                        <div 
                            key={idx} 
                            className="w-10 h-10 rounded-full border-2 border-[#1c2636] bg-cover bg-center bg-white/5 shadow-md" 
                            style={{backgroundImage: `url('${item.image_url}')`}}
                        ></div>
                    ))}
                    {(latestOrder.order_items?.length > 3) && (
                        <div className="w-10 h-10 rounded-full border-2 border-[#1c2636] bg-[#2a3441] flex items-center justify-center text-[9px] text-white font-bold shadow-md">
                            +{latestOrder.order_items.length - 3}
                        </div>
                    )}
                </div>

                {/* СПРАВА: Статус и стрелка */}
                <div className="flex items-center gap-3">
                    <span className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wide border shadow-sm ${status.color}`}>
                        {status.text}
                    </span>
