...
export default function AddressBlock({ 
  ...
  // 1. Фильтруем адреса для курьера (исключаем 5Post)
  const courierAddresses = addresses.filter(addr => {
      const fullString = (addr.street + addr.city + (addr.region || '')).toLowerCase();
      return !fullString.includes('5post');
  });

  // 2. Ищем сохраненные 5Post адреса (если юзер уже заказывал туда)
  const saved5PostAddresses = addresses.filter(addr => {
      const fullString = (addr.street + addr.city + (addr.region || '')).toLowerCase();
      return fullString.includes('5post');
  });

  // Обработчик выбора сохраненного 5Post
  const handleSelectSavedPvz = (addr) => {
      // Пытаемся распарсить строку адреса обратно в объект PVZ (упрощенно)
      // Обычно мы сохраняем строку "5Post: Город, Адрес (Название)"
      // Но для простоты сейчас просто запишем его данные в selectedPvz
      setSelectedPvz({
          id: 'saved_id_' + addr.id, // Фейковый ID, так как реальный ID ПВЗ мы могли не сохранить отдельно в addresses
          city: addr.city || '',
          address: addr.street || addr.address, // В зависимости от того как сохранено
          name: 'Сохраненный пункт',
          postal_code: '000000' // Заглушка, если нет индекса
      });
  };
            ...
            <div className="animate-fade-in space-y-3">
                ...
                {!selectedPvz ? (
                    <div className="space-y-3">
                        {/* 1. Сначала показываем СОХРАНЕННЫЕ ПВЗ */}
                            ...
                            <div className="space-y-2 mb-4">
                                ...
                                {saved5PostAddresses.map(addr => (
                                    <div 
                                        ...
                                        className="p-3 rounded-xl border border-white/10 bg-[#1c2636] cursor-pointer hover:border-primary/50 flex items-center gap-3"
                                    >
                                        <span className="material-symbols-outlined text-primary text-xl">history</span>
                                        <div className="min-w-0">
                                            <p className="text-white text-sm font-bold truncate">{addr.city}, {addr.street}</p>
                                            <p className="text-white/40 text-[10px]">Использовался ранее</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="relative">
                            ...
                            <input 
                               ...
                               placeholder="Найти новый (Город, улица...)"
                               value={pvzQuery}
