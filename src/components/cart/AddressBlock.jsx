  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-2 p-1 bg-white/5 rounded-xl border border-white/5">
           <button 
               onClick={() => { setDeliveryMethod('ПВЗ (5Post)'); setSelectedPvz(null); setPvzQuery(''); }}
            ...
            <div className="animate-fade-in space-y-3">
                ...
                {!selectedPvz ? (
                    <div className="space-y-3">
                        {/* Сохраненные пункты (если есть) */}
                        {saved5PostAddresses.length > 0 && (
                            <div className="space-y-2 mb-4">
                                <p className="text-[10px] text-white/40 font-bold uppercase pl-1">Ранее использованные</p>
                                {saved5PostAddresses.map(addr => (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-white/40 text-[10px]">{addr.full_name}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* ПРОСТОЙ ВВОД (Без умного поиска) */}
                        <div className="relative">
                            <span className="material-symbols-outlined absolute left-3 top-3.5 text-white/40">edit_location</span>
                            <input 
                               ...
                               placeholder="Адрес Пятерочки (Город, Улица, Дом)"
                               value={pvzQuery}
                               onChange={(e) => setPvzQuery(e.target.value)}
                            />
                            {/* Мы убрали рендер списка pvzResults, как ты просил */}
                        </div>
                    </div>
                ) : (
